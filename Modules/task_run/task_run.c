...
void task_init(enum task_index index, int (*task_entry)(struct task_pcb *task), void *user_data,
    ...
    _task_tab[index].user_data = user_data;
}

void task_start(enum task_index index) { _task_tab[index].enable = 1; }

void task_stop(enum task_index index) { _task_tab[index].enable = 0; }

int task_event_send(enum task_index index, uint32_t set) {
    if (_task_tab[index].enable == 0) return -1;
...
void task_process(void) {
    for (int i = 0; i < sizeof(_task_tab) / sizeof(_task_tab[0]); i++) {
        ...
        if ((task->enable == 0) || (task->task_entry == NULL)) continue;

        if ((HAL_GetTick() - task->tick_timeout) >= (HAL_MAX_DELAY / 2)) continue;

        if (task->task_entry) task->task_entry(task);
